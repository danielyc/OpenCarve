...
ease of use (Settings → Design → Simulate → Export) without the carve limits or paywall.
...
- Geometry: clipper2-ts (offset/booleans), d3-delaunay (sampled Voronoi) (medial axis for V-carve), opentype.js (text). Heavy work runs in a Web Worker.
...
6. **V-carve** — medial axis via d3-delaunay (sampled Voronoi), depth from disk radius and bit angle, max-depth flat clearing, chained toolpath. Vitest coverage.
