...
export const BITS: Bit[] = [
  ...
  { id: '60-vbit', name: '60° V-bit, 1/2" (12.7 mm)', type: 'vbit', diameter: 12.7, angle: 60 },
  { id: '90-vbit', name: '90° V-bit, 1/2" (12.7 mm)', type: 'vbit', diameter: 12.7, angle: 90 },
]

// Conservative hobby-router defaults: feeds are the bottom of typical 1/8" bit ranges for belt/lead-screw machines.
...
export const MACHINES: Project['machine'][] = [
  { name: 'Generic GRBL', w: 300, h: 300 },
  { name: '3018', w: 300, h: 180 },
  { name: 'X-Carve 750', w: 750, h: 750 },
  { name: 'X-Carve 1000', w: 1000, h: 1000 },
  { name: 'Shapeoko 4 XL', w: 838, h: 438 },
  { name: 'Shapeoko 4 XXL', w: 838, h: 838 },
  { name: 'Onefinity Woodworker', w: 816, h: 816 },
]
...
// V-bits in the library have no flat, so they get a fixed 1 mm pass.
export function recommendedSettings(material: Material, bit: Bit): CutSettings {
  const stepdown = bit.type === 'vbit' ? 1 : Math.round(Math.min(6, Math.max(0.2, material.stepdownFrac * bit.diameter)) * 10) / 10
  return { feed: material.feed, plunge: material.plunge, stepdown, rpm: material.rpm, safeZ: 5 }
}
