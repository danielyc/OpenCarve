...
export function useSim() {
  ...
  useEffect(() => {
    ...
    if (camBusy) return
    const t = setTimeout(
      () => run({ stock, ops: cam?.ops ?? [], bits: { rough: effectiveBit({ bits, bitOverrides }, 'rough'), ...(bits.detail && { detail: effectiveBit({ bits, bitOverrides }, 'detail') }) } }),
      DEBOUNCE_MS,
    )
    return () => clearTimeout(t)
  }, [cam, camBusy, stock, bits, bitOverrides])
}
