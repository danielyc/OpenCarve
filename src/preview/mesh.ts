...
export function surfaceMesh({ width: W, height: H, cellSize: c, heights, stock }: SimResult): SurfaceMesh {
  ...
  // Two up-facing triangles per cell; cells cut right through are left out so through cuts show as holes.
  ...
  return { positions, normals, colors, index: index.slice(0, n) }
}
